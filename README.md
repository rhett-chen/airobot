...
about camera and object, txt is easy to read and npy is easy to load. 
## Get grasps
After obtainng relevant data, you can use different models to generate grasping pose. For example:

<https://github.com/CZ-Wu/GPNet>
...
## Visualize grasps
The visualization code will draw the point cloud data obtained in step one and the grasping posture obtained in 
 step two in a scene to check the quality of the grasping poses.
You need to set ```data_path, pose_file, method``` in ```options.py```. ```data_path``` directing to data_x dir obtained in step one,
```pose_file``` directing npy/txt file obtained in step two. ```method``` specify which method generates ```pose_file```
because grasp pose representations of different methods are different. At present, only GPNet and 6dof-graspnet are supported.
Run the following command:
```bash
cd scipts/test_grasp
